...
- character :: The smallest component of a written language that has semantic value.
  Distinct from code points and glyphs.

- code point :: A unique numerical value assigned by Unicode to every symbol in
  its standard. An integer ranging from *U+0000* through *U+10FFFF* (0-1,114,111).

- glyph :: The visual representation of a character, separate from its semantic value.

- grapheme cluster :: A sequence of codepoints that represent a grapheme — a minimally
  distinctive unit of text in the context of a particular writing system. What a person
...
A parsing expression that, when compiled, matches and consumes the provided character.
...
The author strongly recommends using *(text "x")* over *(char #\x)*.
...
numerical types, each Scheme character datum maps to a Unicode code point — a unique
integer ranging from 0 through 1,114,111. *Code points map inconsistently to graphemes*
— what a person might perceive to be a character.
