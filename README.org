...
I strongly recommend using *(text "x")* over *(char #\x)*.

*(text "x")* abstracts over code point complexities made explicit by *(char #\x)*.
...
A user-percieved character maps to a grapheme cluster — a collection of one or
more Unicode code points.
...
#+begin_src scheme
  ...
  ;; Capture Function
  (define ABC (compile (capture string-upcase (text "abc"))))
  (ABC "abcdef") ;; -> "ABC"

  ;; Capture Sequence
  (define abc-def (compile (and-then (capture (text "abc")) (capture (text "def")))))
  (abc-def "abcdef") ;; -> ("abc" "def")

  ;; Captures Nested
  (define ABC-abc (compile (capture string-upcase (capture (text "abc")))))
...
- Unlike captures, nested transformations pass their output to enclosing
  transformations combined with any captures outside them but within the
  enclosing transformation. Transformation chaining allows the construction
  of sophisticated parsers.
