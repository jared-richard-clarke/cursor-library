...
Pushes string matched by the sub-expression onto a stack. This stack will later
be returned to the caller. An optional function is applied to the capture before
...
Captures operate by pushing their values onto a stack, which is implemented as a cons list.
...
as computed by parsing expression ~px~.
...
3. A list of captured substrings.
