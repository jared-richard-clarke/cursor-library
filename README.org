...
#+begin_src scheme
  ...
  ;; without "fullstop":
  (define ABC? (compile "ABC"))

  (ABC? "ABC")    ;; -> #t
  (ABC? "ABCDEF") ;; -> #t

  ;; with "fullstop":
  (define ABC? (compile (fullstop "ABC")))
...
In the R6RS standard for Scheme, characters are atomic objects — indivisible data
representing letters, digits, special symbols, and non-graphic control characters.
...
*Scheme characters are not user-perceived characters*. Although distinct from Scheme's
numeric types, each Scheme character datum maps to a Unicode code point — an integer
ranging from ~0~ through ~\xD800~ and ~\xE000~ through ~\x10FFF~.
A user-percieved character might map to one or more Scheme characters.

*** Type ~string~: ~"xyz"~

Transforms into a sequence of character matches. An empty string, ~""~, becomes
an ~empty~ expression, the identity element of a sequence.
...
~and-then~ with one argument transforms into its inner expression.
...
~or-else~ with one argument transforms into its inner expression.
...
#+begin_src scheme
  ...
  (define ABC? (compile ABC)) ;; - equivalent -> (define ABC? (compile "ABC"))
  ...
  (grammar [R1 (and-then (rule R2) #\X)]
           [R2 (and-then (rule R3) #\Y)]
           [R3 (and-then (rule R1) #\Z)])
#+end_src
...
Extracts a copy of the substring matched by pattern ~px~. An optional function
transforms said substring.

- Captures cannot manipulate the output of other captures. Each returns their
  own copy of a substring.

#+begin_src scheme
  ;; Capture
  (define abc (compile (capture "abc")))
  (abc "abcdef") ;; -> "abc"

  ;; Capture Function
  (define ABC (compile (capture string-upcase "abc")))
  (ABC "abcdef") ;; -> "ABC"

  ;; Capture Sequence
  (define abc-def (compile (and-then (capture "abc") (capture "def"))))
  (abc-def "abcdef") ;; -> ("abc" "def")

  ;; Captures Nested
  (define ABC-abc (compile (capture string-upcase (capture "abc"))))
  (ABC-abc "abcdef") ;; -> ("ABC" "abc")
#+end_src

*** ~(transform function px)~

~transform~ applies ~function~ to captures and transformations within pattern ~px~.
...
#+begin_src scheme
  (define digits (capture string->list (repeat+1 (one-of "0123456789"))))
  ...
  (define transform-string
    (compile (transform add-1 (transform convert digits)))

  (transform-string "123") ;; -> 124
#+end_src
